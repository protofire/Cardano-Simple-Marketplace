    ...
    case option of
        BuyNFT ->
            mconcat
                ...
                , Model.payToKey (sellerPaymentPKH datum) $ Model.adaValue (priceOfAsset datum + minADA datum)
...
buyNFT option user contract policy datum = do
    ...
    v1 <- Model.valueAt (sellerPaymentPKH datum)
...
getBackTx option datum contract policy ref =
    case option of
        ...
        WithdrawOcuppedCase ->
            mconcat
                [ Model.spendScript contract ref Withdraw datum
                , Model.payToKey (sellerPaymentPKH datum) $ LedgerValueV1.assetClassValue (sellingTokenAc datum) 1 <> Model.adaValue (minADA datum)
                , Model.mintValue policy (PolicyRedeemerBurnID PolicyRedeemerBurnIDType) (LedgerValueV1.assetClassValue (policyIdAc datum) (-1))
                ]
