  ...
  return (
    <>
      ...
      <div className="grid gap-y-4 items-center">
          ...
          <WalletApiKey />
        <button key={walletStore.info!.pkh + ' disconnect'} className={styles.walletDisc} onClick={async () => await walletDisconnect(false)}>
            Disconnect Wallet
            <Image className={styles.walletImg} src={DISCONNECT.href} alt={'Disconnect'} width={20} height={20} />
        </button>
      </div>
    </>
  );
