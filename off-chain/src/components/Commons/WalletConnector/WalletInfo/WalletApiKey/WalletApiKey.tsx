      ...
      <Modal isOpen={isOpen} onRequestClose={() => handleClose()} contentLabel="Wallet Api Key" className={styles.modal} overlayClassName={styles.overlay}>
        <div className={styles.walletApiKeyContainer}>
          ...
          {!isLoadedDetails || isLoadingDetails ? (
            ...
            <textarea rows={12} cols={60} value={apiToken} readOnly={true} style={{ fontSize: '8px', background: '#2d3748' }}></textarea>
          )}
            <button onClick={() => handleCopy()} className={styles.buttonCopy}>Copy</button>
            <button onClick={() => handleClose()} className={styles.buttonClose}>Close</button>
